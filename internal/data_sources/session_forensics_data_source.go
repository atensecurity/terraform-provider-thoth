...
var _ datasource.DataSource = &sessionForensicsDataSource{}

type sessionForensicsDataSource struct {
	client *client.Client
}

type sessionForensicsModel struct {
...
func NewSessionForensicsDataSource() datasource.DataSource {
	return &sessionForensicsDataSource{}
}

func (d *sessionForensicsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_session_forensics"
}

func (d *sessionForensicsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Reads full investigation forensics for a specific session.",
		Attributes: map[string]schema.Attribute{
			...
			"agent_id": schema.StringAttribute{
				Computed:    true,
				Description: "Agent identifier associated with the session.",
			},
			"agent_name": schema.StringAttribute{
				Computed:    true,
				Description: "Agent name associated with the session.",
			},
			...
			"response_json": schema.StringAttribute{
				Computed:    true,
				Description: "Full session forensics response payload as JSON.",
			},
		},
	}
}

func (d *sessionForensicsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *sessionForensicsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var state sessionForensicsModel
	...
	result, err := d.client.GetSessionForensics(ctx, sessionID)
	if err != nil {
		resp.Diagnostics.AddError("Error reading session forensics", err.Error())
		return
	}
